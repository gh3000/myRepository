...
//lwip 默认IP设置
//lwipx:lwip控制结构体指针
void lwip_comm_default_ip_set(__lwip_dev *lwipx, __lwip_dev *addr)
{
	u32 sn0;
	...
	{
		//默认远端IP为
		lwipx->remoteip[0]=addr->remoteip[0];	
		lwipx->remoteip[1]=addr->remoteip[1];
		lwipx->remoteip[2]=addr->remoteip[2];
		lwipx->remoteip[3]=addr->remoteip[3];
		...
		//默认本地IP为:192.168.10.30
		lwipx->ip[0]=addr->ip[0];	
		lwipx->ip[1]=addr->ip[1];
		lwipx->ip[2]=addr->ip[2];
		lwipx->ip[3]=addr->ip[3];
		
		//默认子网掩码:255.255.255.0
		lwipx->netmask[0]=addr->netmask[0];	
		lwipx->netmask[1]=addr->netmask[1];
		lwipx->netmask[2]=addr->netmask[2];
		lwipx->netmask[3]=addr->netmask[3];
			//默认网关:192.168.10.1
		lwipx->gateway[0]=addr->gateway[0];	
		lwipx->gateway[1]=addr->gateway[1];
		lwipx->gateway[2]=addr->gateway[2];
		lwipx->gateway[3]=addr->gateway[3];	
		lwipx->dhcpstatus=addr->dhcpstatus;	
	}
} 
...
u8 lwip_comm_init(void)
{
	...
	// set remote PC IP
	//默认远端IP为
		addrPC.remoteip[0]=192;	
		addrPC.remoteip[1]=168;
		addrPC.remoteip[2]=10;
		addrPC.remoteip[3]=200;
		...
		//默认子网掩码:255.255.255.0
		addrPC.netmask[0]=255;	
		addrPC.netmask[1]=255;
		addrPC.netmask[2]=255;
		addrPC.netmask[3]=0;
			//默认网关:192.168.10.1
		addrPC.gateway[0]=192;	
		addrPC.gateway[1]=168;
		addrPC.gateway[2]=10;
		addrPC.gateway[3]=1;	
		addrPC.dhcpstatus=0;	
	
	lwip_comm_default_ip_set(&lwipdev, &addrPC);	//设置默认IP等信息
	
	// set remote PLC IP
	//默认远端IP为
		addrPLC.remoteip[0]=192;	
		addrPLC.remoteip[1]=168;
		addrPLC.remoteip[2]=10;
		addrPLC.remoteip[3]=12;
		...
		//默认子网掩码:255.255.255.0
		addrPLC.netmask[0]=255;	
		addrPLC.netmask[1]=255;
		addrPLC.netmask[2]=255;
		addrPLC.netmask[3]=0;
			//默认网关:192.168.10.1
		addrPLC.gateway[0]=192;	
		addrPLC.gateway[1]=168;
		addrPLC.gateway[2]=10;
		addrPLC.gateway[3]=1;	
		addrPLC.dhcpstatus=0;	
	
	lwip_comm_default_ip_set(&lwipdevPLC, &addrPLC);	//设置默认IP等信息
